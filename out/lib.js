...
class AbstractTokenizer {
    ...
    async readToken(token, position = this.position) {
        const uint8Array = Buffer$4.alloc(token.len);
...
(function (exports) {
	...
	class BufferType {
	    ...
	    get(uint8Array, off) {
	        return Buffer$4.from(uint8Array.subarray(off, off + this.len));
	    }
	}
	...
	class StringType {
	    ...
	    get(uint8Array, offset) {
	        return Buffer$4.from(uint8Array).toString(this.encoding, offset, offset + this.len);
	    }
	}
...
util$d.stringToBytes = string => [...string].map(character => character.charCodeAt(0));
...
*/
util$d.tarHeaderChecksumMatches = (buffer, offset = 0) => {
...
async function fromBuffer(input) {
	if (!(input instanceof Uint8Array || input instanceof ArrayBuffer || Buffer$4.isBuffer(input))) {
		throw new TypeError(`Expected the \`input\` argument to be of type \`Uint8Array\` or \`Buffer\` or \`ArrayBuffer\`, got \`${typeof input}\``);
	}

	const buffer = input instanceof Buffer$4 ? input : Buffer$4.from(input);
...
async function _fromTokenizer(tokenizer) {
	let buffer = Buffer$4.alloc(minimumBytes);
	const bytesRead = 12;
	...
	if (checkString('%PDF')) {
		...
		const buffer = Buffer$4.alloc(Math.min(maxBufferSize, tokenizer.fileInfo.size));
		await tokenizer.readBuffer(buffer, {mayBeLess: true});

		// Check if this is an Adobe Illustrator file
		if (buffer.includes(Buffer$4.from('AIPrivateData'))) {
			return {
				ext: 'ai',
				mime: 'application/postscript'
			};
	...
	if (check([0x30, 0x26, 0xB2, 0x75, 0x8E, 0x66, 0xCF, 0x11, 0xA6, 0xD9])) {
		...
		while (tokenizer.position + 24 < tokenizer.fileInfo.size) {
			...
			if (_check(header.id, [0x91, 0x07, 0xDC, 0xB7, 0xB7, 0xA9, 0xCF, 0x11, 0x8E, 0xE6, 0x00, 0xC0, 0x0C, 0x20, 0x53, 0x65])) {
				...
				const typeId = Buffer$4.alloc(16);
...
const stream = readableStream => new Promise((resolve, reject) => {
	...
	readableStream.once('readable', async () => {
		...
		const chunk = readableStream.read(minimumBytes) || readableStream.read() || Buffer$4.alloc(0);
		try {
...
class ID3v1TagMapper extends GenericTagMapper_1$5.CommonTagMapper {
    constructor() {
...
class CaseInsensitiveTagMap extends GenericTagMapper_1$4.CommonTagMapper {
    constructor(tagTypes, tagMap) {
        const upperCaseMap = {};
...
function decodeString(uint8Array, encoding) {
    ...
    return Buffer$4.from(uint8Array).toString(encoding);
}
...
class ID3v24TagMapper extends CaseInsensitiveTagMap_1$2.CaseInsensitiveTagMap {
    static toRating(popm) {
        return {
            source: popm.email,
            rating: popm.rating > 0 ? (popm.rating - 1) / 254 * GenericTagMapper_1$3.CommonTagMapper.maxRatingScore : undefined
        };
    }
    constructor() {
        ...
        switch (tag.id) {
            case 'UFID': // decode MusicBrainz Recording Id
                if (tag.value.owner_identifier === 'http://musicbrainz.org') {
                    ...
                    tag.value = util$c.decodeString(tag.value.identifier, 'latin1'); // latin1 == iso-8859-1
                }
                break;
            case 'PRIV':
...
class AsfTagMapper extends GenericTagMapper_1$2.CommonTagMapper {
...
class VorbisTagMapper extends GenericTagMapper_1$1.CommonTagMapper {
    static toRating(email, rating) {
        return {
            source: email ? email.toLowerCase() : email,
            rating: parseFloat(rating) * GenericTagMapper_1$1.CommonTagMapper.maxRatingScore
        };
    }
    constructor() {
...
class MetadataCollector {
    ...
    addStreamInfo(streamInfo) {
        debug$o(`streamInfo: type=${type_1$2.TrackType[streamInfo.type]}, codec=${streamInfo.codecName}`);
        this.format.trackInfo.push(streamInfo);
    }
    setFormat(key, value) {
        debug$o(`format: ${key} = ${value}`);
    ...
    addTag(tagType, tagId, value) {
        debug$o(`tag ${tagType}.${tagId} = ${value}`);
    ...
    setGenericTag(tagType, tag) {
        debug$o(`common.${tag.id} = ${tag.value}`);
        ...
        if ((0, GenericTagTypes_1.isSingleton)(tag.id)) {
            if (prio1 <= prio0) {
                ...
                return debug$o(`Ignore native tag (singleton): ${tagType}.${tag.id} = ${tag.value}`);
            }
        }
        else {
            if (prio1 === prio0) {
                if (!(0, GenericTagTypes_1.isUnique)(tag.id) || this.common[tag.id].indexOf(tag.value) === -1) {
                    ...
                    debug$o(`Ignore duplicate value: ${tagType}.${tag.id} = ${tag.value}`);
                }
                ...
                return debug$o(`Ignore native tag (list): ${tagType}.${tag.id} = ${tag.value}`);
            }
        }
        if (this.opts.observer) {
...
(function (exports) {
	...
	exports.FourCcToken = {
	    ...
	    put: (buffer, offset, id) => {
	        const str = Buffer$4.from(id, 'binary');
	        if (str.length !== 4)
...
class APEv2Parser extends BasicParser_1$b.BasicParser {
    ...
    static async findApeFooterOffset(reader, offset) {
        ...
        const apeBuf = Buffer$4.alloc(APEv2Token_1.TagFooter.len);
        ...
        if (tagFooter.ID === 'APETAGEX') {
            debug$n(`APE footer header at offset=${offset}`);
    ...
    async tryParseApeHeader() {
        if (this.tokenizer.fileInfo.size && this.tokenizer.fileInfo.size - this.tokenizer.position < APEv2Token_1.TagFooter.len) {
            debug$n(`No APEv2 header found, end-of-file reached`);
            return;
        }
        ...
        if (footer.ID === preamble) {
            ...
            debug$n(`APEv2 header not found at offset=${this.tokenizer.position}`);
            if (this.tokenizer.fileInfo.size) {
                // Try to read the APEv2 header using just the footer-header
                const remaining = this.tokenizer.fileInfo.size - this.tokenizer.position; // ToDo: take ID3v1 into account
                const buffer = Buffer$4.alloc(remaining);
    ...
    async parseTags(footer) {
        const keyBuffer = Buffer$4.alloc(256); // maximum tag key length
        let bytesRemaining = footer.size - APEv2Token_1.TagFooter.len;
        debug$n(`Parse APE tags at offset=${this.tokenizer.position}, size=${bytesRemaining}`);
        for (let i = 0; i < footer.fields; i++) {
            ...
            let zero = util$b.findZero(keyBuffer, 0, keyBuffer.length);
            ...
            switch (tagItemHeader.flags.dataType) {
                ...
                case APEv2Token_1.DataType.binary: // binary (probably artwork)
                    if (this.options.skipCovers) {
                        ...
                        const picData = Buffer$4.alloc(tagItemHeader.size);
                        await this.tokenizer.readBuffer(picData);
                        zero = util$b.findZero(picData, 0, picData.length);
                        const description = picData.toString('utf8', 0, zero);
                        const data = Buffer$4.from(picData.slice(zero + 1));
                ...
                case APEv2Token_1.DataType.external_info:
                    debug$n(`Ignore external info ${key}`);
                    await this.tokenizer.ignore(tagItemHeader.size);
                    break;
...
(function (exports) {
	...
	const debug_1 = browser$1.exports;
...
const debug_1$l = browser$1.exports;
const Token$r = lib$1;
const util$a = Util;
const ID3v2Token_1$3 = ID3v2Token;
const ID3v1Parser_1$3 = ID3v1Parser;
const debug$m = (0, debug_1$l.default)('music-metadata:id3v2:frame-parser');
...
class FrameParser {
    ...
    readData(uint8Array, type, includeCovers) {
        ...
        switch (type !== 'TXXX' && type[0] === 'T' ? 'T*' : type) {
            ...
            case 'PCST':
                let text;
                try {
                    text = util$a.decodeString(uint8Array.slice(1), encoding).replace(/\x00+$/, '');
                }
                catch (error) {
            ...
            case 'TXXX':
                ...
                output = {
                    ...
                    text: this.splitValue(type, util$a.decodeString(output.data, encoding).replace(/\x00+$/, ''))
                };
                break;
            case 'PIC':
            case 'APIC':
                if (includeCovers) {
                    ...
                    switch (this.major) {
                        case 2:
                            pic.format = util$a.decodeString(uint8Array.slice(offset, offset + 3), 'latin1'); // 'latin1'; // latin1 == iso-8859-1;
                            offset += 3;
                            break;
                        case 3:
                        case 4:
                            fzero = util$a.findZero(uint8Array, offset, length, defaultEnc);
                            pic.format = util$a.decodeString(uint8Array.slice(offset, fzero), defaultEnc);
                            offset = fzero + 1;
                            break;
                        default:
                    ...
                    offset += 1;
                    fzero = util$a.findZero(uint8Array, offset, length, encoding);
                    pic.description = util$a.decodeString(uint8Array.slice(offset, fzero), encoding);
                    offset = fzero + nullTerminatorLength;
                    pic.data = Buffer$4.from(uint8Array.slice(offset, length));
                    output = pic;
                }
                break;
            case 'CNT':
            ...
            case 'SYLT':
                ...
                output = [];
                while (offset < length) {
                    const txt = uint8Array.slice(offset, offset = util$a.findZero(uint8Array, offset, length, encoding));
                    offset += 5; // push offset forward one +  4 byte timestamp
                    output.push(util$a.decodeString(txt, encoding));
                }
                break;
            case 'ULT':
            case 'USLT':
            ...
            case 'COMM':
                offset += 1;
                out.language = util$a.decodeString(uint8Array.slice(offset, offset + 3), defaultEnc);
                offset += 3;
                fzero = util$a.findZero(uint8Array, offset, length, encoding);
                out.description = util$a.decodeString(uint8Array.slice(offset, fzero), encoding);
                offset = fzero + nullTerminatorLength;
                out.text = util$a.decodeString(uint8Array.slice(offset, length), encoding).replace(/\x00+$/, '');
                output = [out];
                break;
            case 'UFID':
            ...
            case 'POPM': // Popularimeter
                fzero = util$a.findZero(uint8Array, offset, length, defaultEnc);
                const email = util$a.decodeString(uint8Array.slice(offset, fzero), defaultEnc);
                offset = fzero + 1;
            ...
            case 'GEOB': { // General encapsulated object
                fzero = util$a.findZero(uint8Array, offset + 1, length, encoding);
                const mimeType = util$a.decodeString(uint8Array.slice(offset + 1, fzero), defaultEnc);
                offset = fzero + 1;
                fzero = util$a.findZero(uint8Array, offset, length - offset, encoding);
                const filename = util$a.decodeString(uint8Array.slice(offset, fzero), defaultEnc);
                offset = fzero + 1;
                fzero = util$a.findZero(uint8Array, offset, length - offset, encoding);
                const description = util$a.decodeString(uint8Array.slice(offset, fzero), defaultEnc);
                output = {
                    type: mimeType,
                    filename,
                    description,
            ...
            case 'WPUB':
                // Decode URL
                output = util$a.decodeString(uint8Array.slice(offset, fzero), defaultEnc);
                break;
            case 'WXXX': {
                // Decode URL
                fzero = util$a.findZero(uint8Array, offset + 1, length, encoding);
                const description = util$a.decodeString(uint8Array.slice(offset + 1, fzero), encoding);
                offset = fzero + (encoding === 'utf16le' ? 2 : 1);
                output = { description, url: util$a.decodeString(uint8Array.slice(offset, length), defaultEnc) };
                break;
            }
            case 'WFD':
            case 'WFED':
                output = util$a.decodeString(uint8Array.slice(offset + 1, util$a.findZero(uint8Array, offset + 1, length, encoding)), encoding);
                break;
            case 'MCDI': {
                // Music CD identifier
    ...
    static readIdentifierAndData(uint8Array, offset, length, encoding) {
        const fzero = util$a.findZero(uint8Array, offset, length, encoding);
        const id = util$a.decodeString(uint8Array.slice(offset, fzero), encoding);
...
class ID3v2Parser {
    ...
    static readFrameFlags(b) {
        return {
            status: {
                tag_alter_preservation: util$9.getBit(b, 0, 6),
                file_alter_preservation: util$9.getBit(b, 0, 5),
                read_only: util$9.getBit(b, 0, 4)
            },
            format: {
                grouping_identity: util$9.getBit(b, 1, 7),
                compression: util$9.getBit(b, 1, 3),
                encryption: util$9.getBit(b, 1, 2),
                unsynchronisation: util$9.getBit(b, 1, 1),
                data_length_indicator: util$9.getBit(b, 1, 0)
            }
        };
    }
    ...
    readFrameHeader(uint8Array, majorVer) {
        ...
        switch (majorVer) {
            case 2:
                header = {
                    id: Buffer$4.from(uint8Array.slice(0, 3)).toString('ascii'),
            ...
            case 4:
                header = {
                    id: Buffer$4.from(uint8Array.slice(0, 4)).toString('ascii'),
...
const Token$n = lib$1;
const debug_1$k = browser$1.exports;
...
const iff = iff$1;
const debug$l = (0, debug_1$k.default)('music-metadata:parser:aiff');
    ...
    async parse() {
        ...
        try {
            while (!this.tokenizer.fileInfo.size || this.tokenizer.fileInfo.size - this.tokenizer.position >= iff.Header.len) {
                debug$l('Reading AIFF chunk at offset=' + this.tokenizer.position);
                const chunkHeader = await this.tokenizer.readToken(iff.Header);
                debug$l(`Chunk id=${chunkHeader.chunkID}`);
...
class GUID {
    ...
    static encode(str) {
        ...
        Buffer$4.from(str.slice(19, 23), "hex").copy(bin, 8);
        Buffer$4.from(str.slice(24), "hex").copy(bin, 10);
        return bin;
    }
    equals(guid) {
...
class AsfUtil {
    ...
    static parseUnicodeAttr(uint8Array) {
        return util$8.stripNulls(util$8.decodeString(uint8Array, 'utf16le'));
    }
...
(function (exports) {
	...
	async function readInformation(tokenizer) {
	    const length = await tokenizer.readNumber(Token.UINT16_LE);
	    const buf = Buffer$4.alloc(length);
	...
	class MetadataObjectState extends State {
	    ...
	    get(uint8Array, off) {
	        const tags = [];
	        const buf = Buffer$4.from(uint8Array);
	...
	class WmPictureToken {
	    ...
	    static fromBase64(base64str) {
	        return this.fromBuffer(Buffer$4.from(base64str, 'base64'));
	    }
...
const debug_1$j = browser$1.exports;
const type_1$1 = type;
const GUID_1 = GUID$1;
const AsfObject = AsfObject$1;
const BasicParser_1$9 = BasicParser$1;
const debug$k = (0, debug_1$j.default)('music-metadata:parser:ASF');
...
class AsfParser extends BasicParser_1$9.BasicParser {
    async parse() {
        ...
        try {
            ...
            debug$k('Error while parsing ASF: %s', err);
        }
    }
    async parseObjectHeader(numberOfObjectHeaders) {
        ...
        do {
            ...
            debug$k('header GUID=%s', header.objectId.str);
            switch (header.objectId.str) {
                ...
                case GUID_1.default.PaddingObject.str:
                    ...
                    debug$k('Padding: %s bytes', header.objectSize - AsfObject.HeaderObjectToken.len);
                    await this.tokenizer.ignore(header.objectSize - AsfObject.HeaderObjectToken.len);
                    break;
                default:
                    this.metadata.addWarning('Ignore ASF-Object-GUID: ' + header.objectId.str);
                    debug$k('Ignore ASF-Object-GUID: %s', header.objectId.str);
...
class VorbisPictureToken {
    ...
    get(buffer, offset) {
        ...
        const data = Buffer$4.from(buffer.slice(offset += 4, offset + picDataLen));
        return {
            type,
            format,
            description,
...
class AbstractID3Parser extends BasicParser_1$8.BasicParser {
    ...
    async parseID3v2() {
        ...
        debug$j('End of ID3v2 header, go to MPEG-parser: pos=%s', this.tokenizer.position);
    ...
    async tryReadId3v2Headers() {
        ...
        if (id3Header.fileIdentifier === 'ID3') {
            debug$j('Found ID3v2 header, pos=%s', this.tokenizer.position);
...
class VorbisDecoder {
    ...
    readStringUtf8() {
        ...
        const value = Buffer$4.from(this.data).toString('utf-8', this.offset, this.offset + len);
        this.offset += len;
        return value;
    }
    parseUserComment() {
...
const Token$j = lib$1;
const debug_1$h = browser$1.exports;
const VorbisDecoder_1$1 = VorbisDecoder$1;
const Vorbis_1$1 = Vorbis;
const debug$i = (0, debug_1$h.default)('music-metadata:parser:ogg:vorbis1');
/**
 * Vorbis 1 Parser.
 * Used by OggParser
 */
class VorbisParser {
    ...
    parsePage(header, pageData) {
        if (header.headerType.firstPage) {
            ...
            if (header.headerType.lastPage || !header.headerType.continued) {
                // Flush page segments
                if (this.pageSegments.length > 0) {
                    const fullPage = Buffer$4.concat(this.pageSegments);
    ...
    flush() {
        this.parseFullPage(Buffer$4.concat(this.pageSegments));
    }
    ...
    addTag(id, value) {
        if (id === 'METADATA_BLOCK_PICTURE' && (typeof value === 'string')) {
            if (this.options.skipCovers) {
                debug$i(`Ignore picture`);
                return;
            }
            value = Vorbis_1$1.VorbisPictureToken.fromBase64(value);
            debug$i(`Push picture: id=${id}, format=${value.format}`);
        }
        else {
            debug$i(`Push tag: id=${id}, value=${value}`);
        }
    ...
    parseFirstPage(header, pageData) {
        ...
        if (commonHeader.packetType === 1) {
            ...
            debug$i("sample-rate=%s[hz], bitrate=%s[b/s], channel-mode=%s", idHeader.sampleRate, idHeader.bitrateNominal, idHeader.channelMode);
        }
        else
    ...
    parseFullPage(pageData) {
        // New page
        const commonHeader = Vorbis_1$1.CommonHeader.get(pageData, 0);
        debug$i("Parse full page: type=%s, byteLength=%s", commonHeader.packetType, pageData.byteLength);
...
const debug_1$g = browser$1.exports;
const util$7 = Util;
...
const debug$h = (0, debug_1$g.default)('music-metadata:parser:FLAC');
/**
...
class FlacParser extends AbstractID3Parser_1$3.AbstractID3Parser {
    ...
    parseDataBlock(blockHeader) {
        debug$h(`blockHeader type=${blockHeader.type}, length=${blockHeader.length}`);
...
Metadata.BlockHeader = {
    ...
    get: (buf, off) => {
        return {
            lastBlock: util$7.getBit(buf, off, 7),
            type: util$7.getBitAllignedNumber(buf, off, 1, 7),
...
Metadata.BlockStreamInfo = {
    ...
    get: (buf, off) => {
        return {
            ...
            channels: util$7.getBitAllignedNumber(buf, off + 12, 4, 3) + 1,
            // bits per sample)-1.
            // FLAC supports from 4 to 32 bits per sample. Currently the reference encoder and decoders only support up to 24 bits per sample.
            bitsPerSample: util$7.getBitAllignedNumber(buf, off + 12, 7, 5) + 1,
            // Total samples in stream.
            // 'Samples' means inter-channel sample, i.e. one second of 44.1Khz audio will have 44100 samples regardless of the number of channels.
            // A value of zero here means the number of total samples is unknown.
            totalSamples: util$7.getBitAllignedNumber(buf, off + 13, 4, 36),
	...
	class DataAtom {
	    ...
	    get(buf, off) {
	        return {
	            ...
	            value: Buffer$4.from(new Token.Uint8ArrayType(this.len - 8).get(buf, off + 8))
	        };
	    }
	}
...
Atom$1.Atom = void 0;
const debug_1$f = browser$1.exports;
const AtomToken$1 = AtomToken$2;
const debug$g = (0, debug_1$f.default)('music-metadata:parser:MP4:Atom');
class Atom {
    ...
    static async readAtom(tokenizer, dataHandler, parent, remaining) {
        ...
        debug$g(`parse atom name=${atomBean.atomPath}, extended=${atomBean.extended}, offset=${offset}, len=${atomBean.header.length}`); //  buf.toString('ascii')
...
const debug_1$e = browser$1.exports;
const Token$i = lib$1;
...
const debug$f = (0, debug_1$e.default)('music-metadata:parser:MP4');
...
class MP4Parser extends BasicParser_1$7.BasicParser {
    ...
    async handleAtom(atom, remaining) {
        ...
        if (this.atomParsers[atom.header.name]) {
            ...
            debug$f(`No parser for atom path=${atom.atomPath}, payload-len=${remaining}, ignoring atom`);
            await this.tokenizer.ignore(remaining);
        }
    }
    getTrackDescription() {
    ...
    async parseValueAtom(tagKey, metaAtom) {
        ...
        switch (dataAtom.type.type) {
            ...
            case 13: // JPEG
                ...
                this.addTag(tagKey, {
                    format: 'image/jpeg',
                    data: Buffer$4.from(dataAtom.value)
                });
                break;
            case 14: // PNG
                if (this.options.skipCovers)
    ...
    parseSoundSampleDescription(sampleDescription) {
        ...
        if (version.version === 0 || version.version === 1) {
            ...
            debug$f(`Warning: sound-sample-description ${version} not implemented`);
        }
        return ssd;
    }
    async parseChapterTrack(chapterTrack, track, len) {
        ...
        for (let i = 0; i < chapterTrack.chunkOffsetTable.length && len > 0; ++i) {
            ...
            debug$f(`Chapter ${i + 1}: ${title}`);
            const chapter = {
                title,
                sampleOffset: this.findSampleOffset(track, this.tokenizer.position)
            };
            debug$f(`Chapter title=${chapter.title}, offset=${chapter.sampleOffset}/${this.tracks[0].duration}`);
    ...
    findSampleOffset(track, chapterOffset) {
        ...
        debug$f(`Total duration=${totalDuration}`);
        let chunkIndex = 0;
...
(function (exports) {
	...
	async function readXingHeader(tokenizer) {
	    ...
	    if (flags.toc) {
	        xingInfoTag.toc = Buffer$4.alloc(100);
...
const debug_1$d = browser$1.exports;
const common = Util;
const AbstractID3Parser_1$2 = AbstractID3Parser$1;
const XingTag_1 = XingTag;
const debug$e = (0, debug_1$d.default)('music-metadata:parser:mpeg');
/**
...
class MpegParser extends AbstractID3Parser_1$2.AbstractID3Parser {
    constructor() {
        ...
        this.buf_frame_header = Buffer$4.alloc(4);
        this.syncPeek = {
            buf: Buffer$4.alloc(maxPeekLen),
            len: 0
        };
    }
    /**
    ...
    async postId3v2Parse() {
        ...
        try {
            ...
            if (err instanceof core_1$1.EndOfStreamError) {
                debug$e(`End-of-stream`);
                if (this.calculateEofDuration) {
                    const numberOfSamples = this.frameCount * this.samplesPerFrame;
                    this.metadata.setFormat('numberOfSamples', numberOfSamples);
                    const duration = numberOfSamples / this.metadata.format.sampleRate;
                    debug$e(`Calculate duration at EOF: ${duration} sec.`, duration);
    ...
    finalize() {
        ...
        if (format.duration && this.tokenizer.fileInfo.size) {
            ...
            debug$e("Calculate CBR duration based on file size: %s", duration);
            this.metadata.setFormat('duration', duration);
        }
    }
    async sync() {
        ...
        while (true) {
            ...
            while (true) {
                if (gotFirstSync && (this.syncPeek.buf[bo] & 0xE0) === 0xE0) {
                    ...
                    debug$e(`Sync at offset=${this.tokenizer.position - 1}, frameCount=${this.frameCount}`);
                    if (this.syncFrameCount === this.frameCount) {
                        debug$e(`Re-synced MPEG stream, frameCount=${this.frameCount}`);
                        this.frameCount = 0;
                        this.frame_size = 0;
                    }
    ...
    async parseAudioFrameHeader(header) {
        ...
        if (this.frameCount < 20 * 10000) {
            debug$e('offset=%s MP%s bitrate=%s sample-rate=%s', this.tokenizer.position - 4, header.layer, header.bitrate, header.samplingRate);
        }
        ...
        debug$e(`samples_per_frame=${samples_per_frame}`);
    ...
    async parseAdts(header) {
        ...
        debug$e(`frame-count=${this.frameCount}, size=${header.frameLength} bytes, bit-rate=${bitrate}`);
    ...
    async readXingInfoHeader() {
        ...
        if (infoTag.streamSize) {
            ...
            this.metadata.setFormat('duration', duration);
            debug$e('Get duration from Xing header: %s', this.metadata.format.duration);
            return infoTag;
        }
...
const Token$f = lib$1;
const debug_1$c = browser$1.exports;
const util$6 = Util;
const debug$d = (0, debug_1$c.default)('music-metadata:parser:musepack:sv8');
...
const SH_part3 = {
    ...
    get: (buf, off) => {
        return {
            sampleFrequency: [44100, 48000, 37800, 32000][util$6.getBitAllignedNumber(buf, off, 0, 3)],
            maxUsedBands: util$6.getBitAllignedNumber(buf, off, 3, 5),
            channelCount: util$6.getBitAllignedNumber(buf, off + 1, 0, 4) + 1,
            msUsed: util$6.isBitSet(buf, off + 1, 4),
            audioBlockFrames: util$6.getBitAllignedNumber(buf, off + 1, 5, 3)
        };
    }
};
class StreamReader {
    ...
    async readStreamHeader(size) {
        const streamHeader = {};
        debug$d(`Reading SH at offset=${this.tokenizer.position}`);
        const part1 = await this.tokenizer.readToken(SH_part1);
        size -= SH_part1.len;
        Object.assign(streamHeader, part1);
        debug$d(`SH.streamVersion = ${part1.streamVersion}`);
...
class MpcSv8Parser extends BasicParser_1$6.BasicParser {
    ...
    async parsePacket() {
        ...
        do {
            ...
            debug$c(`packet-header key=${header.key}, payloadLength=${header.payloadLength}`);
            switch (header.key) {
...
StreamVersion7.Header = {
    ...
    get: (buf, off) => {
        const header = {
            // word 0
            signature: Buffer$4.from(buf).toString('latin1', off, off + 3),
            // versionIndex number * 1000 (3.81 = 3810) (remember that 4-byte alignment causes this to take 4-bytes)
            streamMinorVersion: util$5.getBitAllignedNumber(buf, off + 3, 0, 4),
            streamMajorVersion: util$5.getBitAllignedNumber(buf, off + 3, 4, 4),
            // word 1
            frameCount: Token$d.UINT32_LE.get(buf, off + 4),
            // word 2
            maxLevel: Token$d.UINT16_LE.get(buf, off + 8),
            sampleFrequency: [44100, 48000, 37800, 32000][util$5.getBitAllignedNumber(buf, off + 10, 0, 2)],
            link: util$5.getBitAllignedNumber(buf, off + 10, 2, 2),
            profile: util$5.getBitAllignedNumber(buf, off + 10, 4, 4),
            maxBand: util$5.getBitAllignedNumber(buf, off + 11, 0, 6),
            intensityStereo: util$5.isBitSet(buf, off + 11, 6),
            midSideStereo: util$5.isBitSet(buf, off + 11, 7),
            // word 3
            ...
            trueGapless: util$5.isBitSet(buf, off + 23, 0)
        };
...
const debug_1$a = browser$1.exports;
...
const debug$b = (0, debug_1$a.default)('music-metadata:parser:musepack');
class MpcSv7Parser extends BasicParser_1$5.BasicParser {
    ...
    async parse() {
        ...
        debug$b(`stream-version=${header.streamMajorVersion}.${header.streamMinorVersion}`);
        ...
        debug$b(`End of audio stream, switching to APEv2, offset=${this.tokenizer.position}`);
...
class MusepackParser extends AbstractID3Parser_1$1.AbstractID3Parser {
    async postId3v2Parse() {
        ...
        switch (signature) {
            case 'MP+': {
                debug$a('Musepack stream-version 7');
                mpcParser = new MpcSv7Parser_1.MpcSv7Parser();
                break;
            }
            case 'MPC': {
                debug$a('Musepack stream-version 8');
...
Speex$1.Header = {
    ...
    get: (buf, off) => {
        return {
            ...
            version: util$4.trimRightNull(new Token$9.StringType(20, 'ascii').get(buf, off + 8)),
...
const debug_1$8 = browser$1.exports;
const VorbisParser_1$1 = VorbisParser$1;
const Speex = Speex$1;
const debug$9 = (0, debug_1$8.default)('music-metadata:parser:ogg:speex');
/**
 * Speex, RFC 5574
 * Ref:
...
const debug_1$7 = browser$1.exports;
const Theora_1 = Theora;
const debug$8 = (0, debug_1$7.default)('music-metadata:parser:ogg:theora');
/**
 * Ref:
...
class TheoraParser {
    ...
    calculateDuration(header) {
        debug$8('duration calculation not implemented');
    }
    /**
...
const Token$7 = lib$1;
const core_1 = core$1;
const debug_1$6 = browser$1.exports;
const util$3 = Util;
...
const debug$7 = (0, debug_1$6.default)('music-metadata:parser:ogg');
class SegmentTable {
    constructor(header) {
...
class OggParser extends BasicParser_1$4.BasicParser {
     ...
     */
    async parse() {
        debug$7('pos=%s, parsePage()', this.tokenizer.position);
        try {
            let header;
            do {
                ...
                debug$7('page#=%s, Ogg.id=%s', header.pageSequenceNo, header.capturePattern);
                const segmentTable = await this.tokenizer.readToken(new SegmentTable(header));
                debug$7('totalPageSize=%s', segmentTable.totalPageSize);
                const pageData = await this.tokenizer.readToken(new Token$7.Uint8ArrayType(segmentTable.totalPageSize));
                debug$7('firstPage=%s, lastPage=%s, continued=%s', header.headerType.firstPage, header.headerType.lastPage, header.headerType.continued);
                if (header.headerType.firstPage) {
                    const id = new Token$7.StringType(7, 'ascii').get(Buffer$4.from(pageData), 0);
                    switch (id) {
                        case '\x01vorbis': // Ogg/Vorbis
                            debug$7('Set page consumer to Ogg/Vorbis');
                            this.pageConsumer = new VorbisParser_1.VorbisParser(this.metadata, this.options);
                            break;
                        case 'OpusHea': // Ogg/Opus
                            debug$7('Set page consumer to Ogg/Opus');
                            this.pageConsumer = new OpusParser_1.OpusParser(this.metadata, this.options, this.tokenizer);
                            break;
                        case 'Speex  ': // Ogg/Speex
                            debug$7('Set page consumer to Ogg/Speex');
                            this.pageConsumer = new SpeexParser_1.SpeexParser(this.metadata, this.options, this.tokenizer);
                            break;
                        case 'fishead':
                        case '\x00theora': // Ogg/Theora
                            debug$7('Set page consumer to Ogg/Theora');
...
OggParser.Header = {
    ...
    get: (buf, off) => {
        return {
            ...
            headerType: {
                continued: util$3.getBit(buf, off + 5, 0),
                firstPage: util$3.getBit(buf, off + 5, 1),
                lastPage: util$3.getBit(buf, off + 5, 2)
            },
...
const Token$4 = lib$1;
const debug_1$5 = browser$1.exports;
...
const debug$6 = (0, debug_1$5.default)('music-metadata:parser:RIFF');
/**
...
class WaveParser extends BasicParser_1$3.BasicParser {
    async parse() {
        ...
        debug$6(`pos=${this.tokenizer.position}, parse: chunkID=${riffHeader.chunkID}`);
    ...
    async readWaveChunk(remaining) {
        while (remaining >= riff.Header.len) {
            ...
            this.header = header;
            debug$6(`pos=${this.tokenizer.position}, readChunk: chunkID=RIFF/WAVE/${header.chunkID}`);
            switch (header.chunkID) {
                case 'LIST':
                ...
                case 'fmt ': // The Util Chunk, non-PCM Formats
                    ...
                    if (!subFormat) {
                        debug$6('WAVE/non-PCM format=' + fmt.wFormatTag);
                ...
                case '\x00\x00\x00\x00': // padding ??
                    debug$6(`Ignore padding chunk: RIFF/${header.chunkID} of ${header.chunkSize} bytes`);
                    this.metadata.addWarning('Ignore chunk: RIFF/' + header.chunkID);
                    await this.tokenizer.ignore(header.chunkSize);
                    break;
                default:
                    debug$6(`Ignore chunk: RIFF/${header.chunkID} of ${header.chunkSize} bytes`);
                    this.metadata.addWarning('Ignore chunk: RIFF/' + header.chunkID);
                    await this.tokenizer.ignore(header.chunkSize);
            }
            if (this.header.chunkSize % 2 === 1) {
                debug$6('Read odd padding byte'); // https://wiki.multimedia.cx/index.php/RIFF
                await this.tokenizer.ignore(1);
            }
        }
    }
    async parseListTag(listHeader) {
        ...
        debug$6('pos=%s, parseListTag: chunkID=RIFF/WAVE/LIST/%s', this.tokenizer.position, listType);
        switch (listType) {
            case 'INFO':
            ...
            default:
                ...
                debug$6('Ignoring chunkID=RIFF/WAVE/LIST/' + listType);
    ...
    async parseRiffInfoTags(chunkSize) {
        while (chunkSize >= 8) {
            ...
            this.addTag(header.chunkID, util$2.stripNulls(value));
...
const debug_1$4 = browser$1.exports;
const debug$5 = (0, debug_1$4.default)('music-metadata:parser:WavPack');
/**
 * WavPack Parser
 */
class WavPackParser extends BasicParser_1$2.BasicParser {
    ...
    async parseWavPackBlocks() {
        do {
            ...
            debug$5(`WavPack header blockIndex=${header.blockIndex}, len=${WavPackToken_1.WavPack.BlockHeaderToken.len}`);
    ...
    async parseMetadataSubBlock(header, remainingLength) {
        while (remainingLength > WavPackToken_1.WavPack.MetadataIdToken.len) {
            ...
            const data = Buffer$4.alloc(dataSizeInWords * 2 - (id.isOddSize ? 1 : 0));
            await this.tokenizer.readBuffer(data);
            debug$5(`Metadata Sub-Blocks functionId=0x${id.functionId.toString(16)}, id.largeBlock=${id.largeBlock},data-size=${data.length}`);
            switch (id.functionId) {
                case 0x0: // ID_DUMMY: could be used to pad WavPack blocks
                    break;
                case 0xe: // ID_DSD_BLOCK
                    debug$5('ID_DSD_BLOCK');
                ...
                case 0x24: // ID_ALT_TRAILER: maybe used to embed original ID3 tag header
                    debug$5('ID_ALT_TRAILER: trailer for non-wav files');
                    break;
                case 0x26: // ID_MD5_CHECKSUM
                    this.metadata.setFormat('audioMD5', data);
                    break;
                case 0x2f: // ID_BLOCK_CHECKSUM
                    debug$5(`ID_BLOCK_CHECKSUM: checksum=${data.toString('hex')}`);
                    break;
                default:
                    debug$5(`Ignore unsupported meta-sub-block-id functionId=0x${id.functionId.toString(16)}`);
                    break;
            }
            ...
            debug$5(`remainingLength=${remainingLength}`);
            if (id.isOddSize)
...
class DsfParser extends AbstractID3Parser_1.AbstractID3Parser {
    async postId3v2Parse() {
        ...
        if (dsdChunk.metadataPointer === BigInt(0)) {
            debug$4(`No ID3v2 tag present`);
        }
        else {
            debug$4(`expect ID3v2 at offset=${dsdChunk.metadataPointer}`);
    ...
    async parseChunks(bytesRemaining) {
        while (bytesRemaining >= DsfChunk_1.ChunkHeader.len) {
            const chunkHeader = await this.tokenizer.readToken(DsfChunk_1.ChunkHeader);
            debug$4(`Parsing chunk name=${chunkHeader.id} size=${chunkHeader.size}`);
...
const Token = lib$1;
const debug_1$2 = browser$1.exports;
...
const debug$3 = (0, debug_1$2.default)('music-metadata:parser:aiff');
/**
...
class DsdiffParser extends BasicParser_1$1.BasicParser {
    ...
    async readFmt8Chunks(remainingSize) {
        while (remainingSize >= DsdiffToken_1.ChunkHeader64.len) {
            ...
            debug$3(`Chunk id=${chunkHeader.chunkID}`);
            await this.readData(chunkHeader);
            remainingSize -= (BigInt(DsdiffToken_1.ChunkHeader64.len) + chunkHeader.chunkSize);
        }
    }
    async readData(header) {
        debug$3(`Reading data of chunk[ID=${header.chunkID}, size=${header.chunkSize}]`);
        const p0 = this.tokenizer.position;
        switch (header.chunkID.trim()) {
            case 'FVER': // 3.1 FORMAT VERSION CHUNK
                const version = await this.tokenizer.readToken(Token.UINT32_LE);
                debug$3(`DSDIFF version=${version}`);
                break;
            ...
            case 'ID3': // Unofficial ID3 tag support
                ...
                break;
            default:
                debug$3(`Ignore chunk[ID=${header.chunkID}, size=${header.chunkSize}]`);
                break;
            case 'DSD':
        ...
        if (remaining > 0) {
            debug$3(`After Parsing chunk, remaining ${remaining} bytes`);
            await this.tokenizer.ignore(Number(remaining));
        }
    }
    async handleSoundPropertyChunks(remainingSize) {
        debug$3(`Parsing sound-property-chunks, remainingSize=${remainingSize}`);
        while (remainingSize > 0) {
            const sndPropHeader = await this.tokenizer.readToken(DsdiffToken_1.ChunkHeader64);
            debug$3(`Sound-property-chunk[ID=${sndPropHeader.chunkID}, size=${sndPropHeader.chunkSize}]`);
            const p0 = this.tokenizer.position;
            switch (sndPropHeader.chunkID.trim()) {
                ...
                case 'ABSS': // 3.2.4 Absolute Start Time Chunk
                    ...
                    debug$3(`ABSS ${hours}:${minutes}:${seconds}.${samples}`);
                    break;
                case 'LSCO': // 3.2.5 Loudspeaker Configuration Chunk
                    const lsConfig = await this.tokenizer.readToken(Token.UINT16_BE);
                    debug$3(`LSCO lsConfig=${lsConfig}`);
                    break;
                case 'COMT':
                default:
                    debug$3(`Unknown sound-property-chunk[ID=${sndPropHeader.chunkID}, size=${sndPropHeader.chunkSize}]`);
                    await this.tokenizer.ignore(Number(sndPropHeader.chunkSize));
            }
            const remaining = sndPropHeader.chunkSize - BigInt(this.tokenizer.position - p0);
            if (remaining > 0) {
                debug$3(`After Parsing sound-property-chunk ${sndPropHeader.chunkSize}, remaining ${remaining} bytes`);
                await this.tokenizer.ignore(Number(remaining));
            }
...
const token_types_1 = lib$1;
const debug_1$1 = browser$1.exports;
const BasicParser_1 = BasicParser$1;
const types_1 = types;
const matroskaDtd = MatroskaDtd;
const debug$2 = (0, debug_1$1.default)('music-metadata:parser:matroska');
/**
...
class MatroskaParser extends BasicParser_1.BasicParser {
    ...
    async parseContainer(container, posDone, path) {
        ...
        while (this.tokenizer.position < posDone) {
            ...
            if (type) {
                debug$2(`Element: name=${type.name}, container=${!!type.container}`);
                if (type.container) {
                ...
                switch (element.id) {
                    case 0xec: // void
                        ...
                        break;
                    default:
                        debug$2(`parseEbml: path=${path.join('/')}, unknown element: id=${element.id.toString(16)}`);
                        this.padding += element.len;
                        await this.tokenizer.ignore(element.len);
                }
            }
    ...
    async readBuffer(e) {
        const buf = Buffer$4.alloc(e.len);
        await this.tokenizer.readBuffer(buf);
        return buf;
    }
    addTag(tagId, value) {
...
const debug$1 = (0, debug_1.default)('music-metadata:parser:factory');
...
class ParserFactory {
    ...
    static async parseOnContentType(tokenizer, opts) {
        ...
        if (!parserId) {
            debug$1('No parser found for MIME-type / extension: ' + mimeType);
        }
...
var streamBrowser = require$$1.EventEmitter;

var require$$3 = /*@__PURE__*/getAugmentedNamespace(bufferEs6);
...
var require$$4 = /*@__PURE__*/getAugmentedNamespace(util$1);
...
(function (exports) {
	...
	const initDebug = browser$1.exports;
	const mm = core$2;
